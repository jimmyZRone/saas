				...
				if(house_type == 2){  //集中式
					if(rental_way == 2){    //整租
						if(floor != '' && room_type != '' && money != '' && area != '' && is_pic != 0){
							...
							window.parent.mask.noDialogShowModle('提示信息','请先完善房源信息',det);
						}
...
//					house_type = $(".introduce-tab").attr("house_type"),
//					rental_way = $(".introduce-tab").attr("rental_way");
//				var type = "POST";
//				var url = $(".st-detail-btn").attr("url");
//				var data = {
//					"key": filename,
//					"room_id": room_id,
...
//				});
		 ...
		 *
		 */
		var detailEvent = window.detailEvent = {
			...
			detailShow: function(el){
				...
				ajax.doAjax(type,url,data,function(data){
					if(data.status == 1){
						...
						$('.head-auto', parent.document).hide();
						$('.center', parent.document).hide();
						
						$(".main-show",parent.document).animate({
							"scrollTop": 0
						},1,function(){
//							$(".main-show", parent.document).css("overflow-y","hidden");
						});
						...
						//详情滚动条
						var _h = $(window.parent).height() - 200;
						$(".detail-main-box").css({
							"height": _h,
							"overflow-y": "auto"
						});
						$(window).resize(function(){
//							$(".main-show", parent.document).css("overflow-y","hidden");
							var height = $(window.parent).height() - 200;
							$(".detail-main-box").css({
								"height": height,
								"overflow-y": "auto"
							});
						...
						$(".money").text(money);
//						if(floor == ''){
//							$(".floor").html('<input type="text" class="ipt ipt-100 mr5" placeholder="" /><i>*</i>');
//						}else{
//							
//						}
//						if(area == ''){
//							$(".area").html('<input type="text" class="ipt ipt-100 mr5" placeholder="" /><i>*</i>');
//						}else{
//							
//						}
//						if(money == ''){
//							$(".money").html('<input type="text" class="ipt ipt-100 mr5" placeholder="" /><i>*</i>');
//						}else{
//								
//						}
						if(dead_line != 0){
					...
					if($(this).parents("li").hasClass("is_recom")){
						if(obj.hasClass("checked")){
							...
							if(pic_length < 4){
								...
								window.parent.mask.noDialogShowModle('提示信息','您已经推荐了四个精品房清，不能再推荐');
							}
						}
		...
		var returnEvent = function(){
			$(".return").off("click").on("click",function(){
				...
				$('.head-auto', parent.document).show();
				$('.center', parent.document).show();
...
//				var res = $(".hide-msg").data("hid");
			...
			callback: function(data){
				if(data.status == 1){
					...
					$(".st-detail").siblings().show();
					$('.head-auto', parent.document).show();
					$('.center', parent.document).show();
					
					$(".main-show", parent.document).css({
						"overflow-y": "auto",
					});
			...
			checkUI: function(callfun){
				$('.st-detail').Validform({
		            ...
		            callback: function(){
						...
						var money = $(".money").text();
		            	if(len <= 0){
		            		window.parent.mask.noDialogShowModle('提示信息','房间图片不能少于1张');
		            	}else if(room_type == '--' || area == '--' || money == '--'){
		            		window.parent.mask.noDialogShowModle('提示信息','请先完善信息');
		            	}else{
		...
		var houseMsgSubmit = {
			submitForm: function(){
				var house_id = [], room_id = [], house_type= [], rental_way = [], house_name = [], community_id = [], community_name = [], address = [],
					rent = [], area_id = [], business_id = [], area = [], floor = [], city_id = [], status = [], is_recom = [], expire_time = [], online_time = [], flat_id= [], wxxzid = [], cover = [];
...
//			            		window.parent.mask.dialogShowModle('提示信息','请至少选择一个房源');
//			            	}else{
