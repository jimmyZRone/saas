    ...
    class Model
    {
        ...
        function query($sql)
        {
            ...
            if (!$result)
            {
                ...
                {
                    $text .= "{$info['file']}第{$info['line']}行 > ";
                }
                var_dump($text . mysqli_error($this->link));
            }
            return $result;
        }
        ...
        public function addAll($dataList , $options = array() , $replace = false)
        {
            if (empty($dataList))
            {
                $this->error = $this->L('_DATA_TYPE_INVALID_');
                return false;
            }
            // 分析表达式
            $options = $this->_parseOptions($options);
        ...
        protected function parseJoin($join)
        {
            ...
            $joinStr = @preg_replace("/__([A-Z_-]+)__/esU" , $this->C("DB_PREFIX") . ".strtolower('$1')" , $joinStr);
            return $joinStr;
        }
