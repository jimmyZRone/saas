 ...
 *
 */
class SystemConfig extends \Common\Model\Erp
{
	...
	public function getFind($list,$key){
		...
		$company_config = $companyModel->get($list.'/'.$key);
		if (!empty($company_config[$list][$key]))
		{
			return $company_config[$list][$key];
		}
	...
	/**
	 * 通过key值查找配置
	 * @author yusj | 最后修改时间 2015年5月4日下午1:34:57
	 */
	public function getDataByKey($key){
		$companyModel = new Company();
		$company_config = $companyModel->getDataByKey($key);
		if (!empty($company_config[$key]))
		{
			return $company_config[$key];
		}
		$list = array();
		$retVal = $this->getData(array('key'=>$key));
		//整理最后的返回格式
		$_retVal = array();
		foreach ($retVal as $value){
			if(!isset($_retVal[$value['list']])){
